...
// Note: Assuming 'sonner' or similar toast lib is installed or I should use a simple alert if not. 
// I will use a simple custom toast or console for now if no lib is found, 
// but user asked for "Show a success toast", 
// I'll check if any toast lib exists. 
// Browsing package.json might be good, but standard alerts are safe for now.
...
export default function QuickOrderTable() {
  ...
  const handleCheckboxChange = (id: string, checked: boolean) => {
    setRows(prev => prev.map(row => {
      if (row.product.id === id) {
        // Auto-deselect logic: if unchecked, maybe reset qty? 
        // User req: "If manual uncheck, quantity should reset or be ignored".
        // Let's reset quantity to 0 to be clear.
  ...
  const handleBulkAdd = async () => {
    ...
    // Optional: Reset form?
    // setRows(prev => prev.map(r => ({...r, quantity: 0, isSelected: false})));
  };
              ...
              >
                ...
                <td className="p-4 text-center">
                  <input
                    ...
                    onKeyDown={(e) => {
                      // Prevent non-numeric chars except needed keys
